"""Visualization of Levenshtein Distance using Manim.

This script demonstrates the dynamic programming approach to computing the
Levenshtein distance between two strings ("kitten" and "sitting").
...
# Add parent directory to path to allow potential shared imports
sys.path.append(os.path.join(os.path.dirname(__file__), ".."))


class LevenshteinDemo(Scene):
    """Manim Scene for demonstrating Levenshtein Distance DP."""
    ...
    def construct(self) -> None:
        ...
        cell_size = 0.8
        # Calculate start position to center the grid somewhat
        # (Approximation)
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                self.play(
                    cells[i][j]["square"].animate.set_fill(YELLOW, opacity=0.5),
                    run_time=0.1,
                )
                self.add(text)
                self.play(
                    cells[i][j]["square"].animate.set_fill(opacity=0),
                    run_time=0.1,
                )
