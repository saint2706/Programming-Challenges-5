...
class Label:
    ...
    def get_rect(self) -> Tuple[float, float, float, float]:
        ...
        if self.position_idx == 0: # Top Right
            return (self.target_x + p, self.target_y + p, self.target_x + p + self.width, self.target_y + p + self.height)
        elif self.position_idx == 1: # Top Left
            return (self.target_x - p - self.width, self.target_y + p, self.target_x - p, self.target_y + p + self.height)
        elif self.position_idx == 2: # Bottom Right
            return (self.target_x + p, self.target_y - p - self.height, self.target_x + p + self.width, self.target_y - p)
        elif self.position_idx == 3: # Bottom Left
            return (self.target_x - p - self.width, self.target_y - p - self.height, self.target_x - p, self.target_y - p)
        return (0,0,0,0)
...
class MapLabeler:
    ...
    def energy(self) -> float:
        ...
        overlap_weight = 1000
        boundary_weight = 500
        pref_weights = [0, 2, 5, 10] # TopRight(0) is best (cost 0), BottomLeft(3) is worst
        ...
        for i in range(n):
            ...
            # Boundary check
            if (r1[0] < self.bounds[0] or r1[2] > self.bounds[2] or
                r1[1] < self.bounds[1] or r1[3] > self.bounds[3]):
