...
```json
[{ "result": "created" }, { "result": "inserted" }, { "result": "Hello World" }]
```
...
_Note: This naive implementation does not auto-balance the tree, so worst-case performance can degrade to $O(n)$ if constructed linearly. Production ropes (e.g., in text editors) use rebalancing (AVL/Red-Black rules)._
