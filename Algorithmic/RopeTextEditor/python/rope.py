...
class RopeNode:
    ...
    def __init__(self, value: str = "", left: Optional['RopeNode'] = None, right: Optional['RopeNode'] = None):
        self.value = value
        self.left = left
        self.right = right
        if left is None:
    ...
    def _collect(self) -> str:
        if self.left is None and self.right is None:
            return self.value
        s = ""
        if self.left: s += self.left._collect()
        if self.right: s += self.right._collect()
        return s
...
class Rope:
    ...
    def concat(self, other: 'Rope') -> 'Rope':
        if not self.root: return other
        if not other.root: return self
        new_root = RopeNode(left=self.root, right=other.root)
        return Rope(new_root)

    def index(self, i: int) -> str:
        if not self.root: raise IndexError("Index out of bounds")
    ...
    def _split_node(self, node: RopeNode, i: int) -> Tuple[Optional[RopeNode], Optional[RopeNode]]:
    ...
    def delete(self, i: int, j: int) -> 'Rope':
        ...
        left, right = self.split(i)
        _, remaining = self.split(j) # Wait, split is destructive? No, it returns new nodes.
