...
class SkipListDemo(Scene):
    def construct(self):
        ...
        levels = 4
        data = {
            3: [0, 1, 2, 3], # Present in all levels (simulated)
            2: [0, 1, 2],
            1: [0, 1],
            0: [0]
        }
        ...
        # Draw nodes
        node_mobjects = {} # Key: (val, level) -> mobject
        ...
        for lvl in range(levels):
            ...
            for next_node in nodes[1:]:
                if next_node["levels"] > lvl:
                    ...
                    arrow = Arrow(start, end, buff=0.1, max_tip_length_to_length_ratio=0.15)
                    self.add(arrow)
                    arrows.append(arrow)
                    current_node = next_node
                    
        # Vertical arrows (optional, usually implicit)
        ...
        pointer = Arrow(start=UP, end=DOWN, color=RED).next_to(node_mobjects[(curr_val, curr_lvl)], UP)
        ...
        while curr_lvl >= 0:
            ...
            if next_node["val"] == "inf" or next_node["val"] > target:
                ...
                # Found!
                self.play(pointer.animate.next_to(node_mobjects[(target, curr_lvl)], UP), run_time=0.5)
                found_mobj = node_mobjects[(target, curr_lvl)]
                self.play(found_mobj[0].animate.set_fill(GREEN, opacity=0.8), run_time=0.5)
                break
            else:
                # Go right
                curr_val = next_node["val"]
                self.play(pointer.animate.next_to(node_mobjects[(curr_val, curr_lvl)], UP), run_time=0.3)
