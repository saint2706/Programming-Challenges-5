...
def main():
    ...
    parser.add_argument(
        "--width", type=int, help="Target width (optional, defaults to original)"
    )
    parser.add_argument(
        "--height", type=int, help="Target height (optional, defaults to original)"
    )
    parser.add_argument(
        "--scale",
        type=float,
        help="Scale factor (e.g., 0.5 for half size). Overrides width/height.",
    )
    ...
    print(
        f"Resizing from {original_width}x{original_height} to {target_width}x{target_height}..."
    )

    try:
        resized_image = seam_carve(
            image, target_width, target_height, verbose=args.verbose
        )
