"""Visualization of Trie (Prefix Tree) operations using Manim.

This script demonstrates the insertion of words into a Trie structure.
...
class TrieDemo(Scene):
    """Manim Scene for demonstrating Trie insertion."""
    ...
    def construct(self) -> None:
        ...
        # Map prefix -> (Mobject, Position)
        nodes: Dict[str, Tuple[Mobject, np.ndarray]] = {"": (root_node, root_pos)}
        edges: List[Line] = []

        def get_pos(prefix: str) -> np.ndarray:
            """Calculate position for a node based on its prefix (pseudo-layout)."""
            depth = len(prefix)
            if depth == 0:
                return root_pos
