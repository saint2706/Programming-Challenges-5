...
function render(weatherData = null, forecastData = null, error = null) {
    app.innerHTML = `
        <div class="container">
            ...
            ${weatherData ? renderCurrentWeather(weatherData) : `
...
async function searchCity(city) {
    ...
    try {
        const weatherData = await fetchWeather(`${API_BASE}/weather?q=${city}&units=${units}&appid=${API_KEY}`);
        const forecastData = await fetchWeather(`${API_BASE}/forecast?q=${city}&units=${units}&appid=${API_KEY}`);
...
function getCurrentLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        async (position) => {
            ...
            try {
                const weatherData = await fetchWeather(`${API_BASE}/weather?lat=${latitude}&lon=${longitude}&units=${units}&appid=${API_KEY}`);
                const forecastData = await fetchWeather(`${API_BASE}/forecast?lat=${latitude}&lon=${longitude}&units=${units}&appid=${API_KEY}`);
                render(weatherData, forecastData);
            } catch (error) {
