  ...
  validateField(field) {
    ...
    for (const rule of rules) {
      ...
      if (!result.isValid) {
        this.showError(
          field,
          result.message || validator.message(label, ruleValue, field),
        );
        return false;
      }
    }
  ...
  validators = {
    ...
    phone: {
      validate: (value) => {
        ...
        const pattern = /^[\d\s\-\+\(\)]+$/;
        return {
          isValid: pattern.test(value) && value.replace(/\D/g, "").length >= 10,
        };
      },
    ...
    password: {
      ...
      message: (label) =>
        `${label} must contain at least 8 characters with uppercase, lowercase, and number`,
    },
