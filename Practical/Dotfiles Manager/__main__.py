...
def install_dotfiles(
    source_dir: str, target_dir: str, backup: bool = True, dry_run: bool = False
):
    """
    ...
    for item in os.listdir(source_dir):
        ...
        if os.path.islink(target_path):
            ...
            if current_src == source_path:
                ...
                logging.info(
                    f"UPDATE: {target_name} linked to {current_src}, relinking..."
                )
                if not dry_run:
            ...
            if backup:
                ...
                logging.warning(
                    f"CONFLICT: {target_name} exists. Skipping (use --backup to move it)."
                )
                continue
...
def main():
    ...
    parser.add_argument(
        "--no-backup", action="store_true", help="Do not backup existing files"
    )
    ...
    install_dotfiles(
        args.source, args.target, backup=not args.no_backup, dry_run=args.dry_run
    )
