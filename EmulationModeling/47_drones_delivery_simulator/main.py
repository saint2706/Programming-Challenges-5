...
class DroneSimulation(DiscreteEventSimulation):
    def __init__(self, config: DroneConfig):
        ...
        self.config = config
        self.visualizer = SimulationVisualizer(output_dir=f"EmulationModeling/47_drones_delivery_simulator/{config.output_dir}")
    ...
    def setup_world(self):
        ...
        for _ in range(50):
            x, y = random.randint(0, self.config.map_size-1), random.randint(0, self.config.map_size-1)
            self.grid[x, y] = 1
        ...
        for i in range(self.config.num_orders):
            ...
            self.orders.append({"id": i, "start": start, "end": end, "status": "pending"})
    ...
    def get_path(self, start, end):
        try:
            return nx.astar_path(self.graph, start, end, heuristic=lambda u, v: math.hypot(u[0]-v[0], u[1]-v[1]))
