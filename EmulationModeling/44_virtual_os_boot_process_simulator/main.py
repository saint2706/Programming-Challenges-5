...
class BootSimulation(DiscreteEventSimulation):
    def __init__(self, config: BootConfig):
        ...
        self.visualizer = SimulationVisualizer(output_dir=f"EmulationModeling/44_virtual_os_boot_process_simulator/{config.output_dir}")
        self.setup_dag()
    ...
    def run_stage(self, stage: BootStage):
        ...
        if stage.dependencies:
            yield simpy.AllOf(self.env, [self.stages[d].event for d in stage.dependencies])
    ...
    def snapshot(self):
        ...
        nx.draw(self.graph, pos, with_labels=True, node_color=colors, node_size=1500, font_weight="bold", ax=ax)
