...
def diffusion_step_2d(
    grid: np.ndarray, diff: float, dt: float, dx: float, dy: float
) -> np.ndarray:
    """
...
@numba.jit(nopython=True)
def advect_step_2d(
    grid: np.ndarray, u: np.ndarray, v: np.ndarray, dt: float, dx: float, dy: float
) -> np.ndarray:
    """
    ...
    for i in range(1, ny - 1):
        for j in range(1, nx - 1):
            ...
            grad_x = (
                ...
                if u[i, j] > 0
                else (grid[i, j + 1] - grid[i, j]) / dx
            )
            grad_y = (
                (grid[i, j] - grid[i - 1, j]) / dy
                if v[i, j] > 0
                else (grid[i + 1, j] - grid[i, j]) / dy
            )
