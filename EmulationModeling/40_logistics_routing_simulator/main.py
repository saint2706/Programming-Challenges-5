...
class LogisticsSimulation(DiscreteEventSimulation):
    def __init__(self, config: LogisticsConfig):
        ...
        self.customers = []
        self.trucks = []
        self.visualizer = SimulationVisualizer(output_dir=f"EmulationModeling/40_logistics_routing_simulator/{config.output_dir}")
        self.setup_world()

    def setup_world(self):
        # Create Depots
        for i in range(self.config.num_depots):
            pos = (random.randint(0, self.config.map_size), random.randint(0, self.config.map_size))
        ...
        for i in range(self.config.num_customers):
            ...
            self.customers.append({"id": f"C{i}", "pos": pos, "demand": demand, "served": False})
    ...
    def truck_process(self, truck_idx):
        ...
        while True:
            ...
            potential_customers = [c for c in self.customers if not c["served"] and c["demand"] + truck["load"] <= truck["capacity"]]

            target = None
            if potential_customers:
                # Find nearest
                potential_customers.sort(key=lambda c: math.hypot(c["pos"][0] - current_pos[0], c["pos"][1] - current_pos[1]))
                ...
                depots_sorted = sorted(self.depots, key=lambda d: math.hypot(d["pos"][0] - current_pos[0], d["pos"][1] - current_pos[1]))
                target = depots_sorted[0]
                action = "refill"

            # Move to target
            dist = math.hypot(target["pos"][0] - current_pos[0], target["pos"][1] - current_pos[1])
    ...
    def snapshot(self):
        ...
        # Draw Depots
        nx.draw_networkx_nodes(self.graph, pos, nodelist=[d["id"] for d in self.depots], node_color="red", node_size=100, label="Depot", ax=ax)
        ...
        nx.draw_networkx_nodes(self.graph, pos, nodelist=unserved, node_color="blue", node_size=50, label="Customer (Wait)", ax=ax)
        nx.draw_networkx_nodes(self.graph, pos, nodelist=served, node_color="green", node_size=50, label="Customer (Done)", ax=ax)
