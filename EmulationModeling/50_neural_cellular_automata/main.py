...
class CAModel(nn.Module):
    def __init__(self, channel_n, hidden_n):
        ...
        self.fc0 = nn.Linear(channel_n * 3, hidden_n) # Sobel filters (3x3 input context)
    ...
    def forward(self, x):
        ...
        def depthwise(filt):
            return torch.nn.functional.conv2d(x, filt.repeat(self.channel_n, 1, 1, 1), groups=self.channel_n, padding=1)
...
class NCASimulation:
    def __init__(self, config: NCAConfig):
        self.config = config
        self.visualizer = SimulationVisualizer(output_dir=f"EmulationModeling/50_neural_cellular_automata/{config.output_dir}")
        ...
        self.state = torch.zeros(1, config.channels, config.grid_size, config.grid_size, device=self.device)
