...
from main import Unit, Game, GameState, GRID_SIZE
...
def test_ai_pathfinding():
    ...
    initial_distance = abs(enemy.grid_x - game.player.grid_x) + abs(enemy.grid_y - game.player.grid_y)
    
    # Find best move
    best_move = game.find_best_move(enemy)
    
    if best_move:
        new_distance = abs(best_move[0] - game.player.grid_x) + abs(best_move[1] - game.player.grid_y)
