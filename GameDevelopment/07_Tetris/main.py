...
SHAPES = [S, Z, I_SHAPE, O_SHAPE, J, L, T]
...
class Tetris:
    ...
    def valid_space(self, piece, x_offset=0, y_offset=0):
        accepted_pos = [
            [(j, i) for j in range(GRID_WIDTH) if self.grid[i][j] == (0, 0, 0)]
    ...
    def draw_grid(self, surface):
        ...
        pygame.draw.rect(
            surface, RED, (TOP_LEFT_X, TOP_LEFT_Y, PLAY_WIDTH, PLAY_HEIGHT), 4
        )
    ...
    def draw_window(self, surface):
        ...
        surface.blit(
            score_label,
            (TOP_LEFT_X + PLAY_WIDTH + 50, TOP_LEFT_Y + PLAY_HEIGHT / 2 - 100),
        )

        level_label = font_small.render(f"Level: {self.level}", 1, WHITE)
        surface.blit(
            level_label,
            (TOP_LEFT_X + PLAY_WIDTH + 50, TOP_LEFT_Y + PLAY_HEIGHT / 2 - 70),
        )
        ...
        while self.valid_space(
            self.current_piece, y_offset=ghost_y - self.current_piece.y + 1
        ):
            ghost_y += 1
            ...
            for j, column in enumerate(row):
                if column == "0":
                    pygame.draw.rect(
                        ...
                        shape.color,
                        (
                            TOP_LEFT_X + PLAY_WIDTH + 50 + j * BLOCK_SIZE,
                            TOP_LEFT_Y + 30 + i * BLOCK_SIZE,
                            BLOCK_SIZE,
                            BLOCK_SIZE,
                        ),
                        0,
