...
class Puzzle:
    ...
    def get_valid_moves(self):
        ...
        if x < GRID_SIZE - 1: moves.append((x + 1, y)) # Right
        if y > 0: moves.append((x, y - 1)) # Up
        if y < GRID_SIZE - 1: moves.append((x, y + 1)) # Down
        return moves
    ...
    def move(self, pos):
        ...
        if abs(ex - tx) + abs(ey - ty) == 1:
            # Swap
            self.tiles[ey][ex], self.tiles[ty][tx] = self.tiles[ty][tx], self.tiles[ey][ex]
