...
pytest.importorskip("pygame", reason="pygame is required to run the game module")
os.environ.setdefault("SDL_VIDEODRIVER", "dummy")

_main = importlib.import_module("GameDevelopment.11_TurnBasedStrategy.main")
GRID_SIZE = _main.GRID_SIZE
Game = _main.Game
GameState = _main.GameState
Unit = _main.Unit
...
def test_unit_movement():
    ...
    assert unit.can_move_to(6, 5, grid)  # 1 tile away
    assert unit.can_move_to(7, 5, grid)  # 2 tiles away
    assert unit.can_move_to(8, 5, grid)  # 3 tiles away (max range)
    assert not unit.can_move_to(9, 5, grid)  # 4 tiles away (too far)

    # Cannot move to occupied tile  (grid is [y][x])
    grid[5][6] = Unit(6, 5, (0, 0, 255), "Blocker", is_player=False)
    assert not unit.can_move_to(6, 5, grid)
    ...
    grid[5][5] = None
    grid[5][6] = unit
    assert not unit.can_move_to(7, 5, grid)  # Has already moved
