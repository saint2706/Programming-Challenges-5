...
def test_forward_shapes() -> None:
    ...
    model = Seq2Seq(encoder=encoder, decoder=decoder, pad_idx=vocab_src.pad_idx, device=torch.device("cpu"))
...
def test_overfits_tiny_dataset() -> None:
    model, vocab_src, vocab_tgt = train(
        num_epochs=25, embed_dim=16, hidden_dim=32, batch_size=4, teacher_forcing_ratio=0.9, device=torch.device("cpu")
    )
